"""
金融理财Agent

提供金融投资、理财规划、市场分析等专业咨询服务。
这是一个具有高商业价值的Agent，可对接金融API和LLM。
"""
from typing import Dict, Any, Optional
from datetime import datetime
from src.agents import BaseAgent, Message, ChatContext, AgentResponse, SQLiteMemoryStore


class FinanceAgent(BaseAgent):
    """
    金融理财Agent - 提供专业金融咨询
    
    专长领域:
    - 投资理财：股票、基金、债券咨询
    - 市场分析：行情走势、投资建议
    - 个人财务：预算管理、储蓄规划
    - 加密货币：数字资产咨询
    
    适用场景:
    - "帮我分析一下近期的股市走势"
    - "我该如何规划退休金？"
    - "介绍一些适合新手的理财产品"
    """
    
    def __init__(self, memory_store=None, llm_provider=None):
        """
        初始化金融理财Agent
        
        Args:
            memory_store: 记忆存储实例
            llm_provider: LLM提供者（用于生成专业回复）
        """
        self._name = "FinanceAgent"
        self._description = (
            "提供金融理财咨询和投资建议。"
            "涵盖股票、基金、个人财务规划等领域，"
            "帮助用户做出明智的财务决策。"
        )
        self._memory = memory_store or SQLiteMemoryStore()
        self._llm_provider = llm_provider
        
        # 金融相关关键词
        self._finance_keywords = [
            # 投资类
            "股票", "基金", "债券", "投资", "理财", "证券",
            "stock", "fund", "bond", "invest", "investment",
            "portfolio", "dividend", "收益", "回报", "分红",
            
            # 市场类
            "行情", "走势", "涨", "跌", "牛市", "熊市",
            "market", "bull", "bear", "trend", "analysis",
            
            # 个人财务
            "储蓄", "预算", "退休", "保险", "贷款", "房贷",
            "savings", "budget", "retirement", "insurance", "loan", "mortgage",
            
            # 加密货币
            "比特币", "以太坊", "加密货币", "数字货币", "币圈",
            "bitcoin", "ethereum", "crypto", "cryptocurrency", "blockchain",
            
            # 财务规划
            "理财规划", "财务自由", "资产配置", "风险管理",
            "financial planning", "wealth", "asset allocation",
        ]
        
        # 系统提示词
        self._system_prompt = """你是一位专业的金融理财顾问，拥有丰富的投资经验和财务规划知识。

你的职责：
1. 提供专业的金融知识和投资建议
2. 解答用户关于股票、基金、债券等投资产品的问题
3. 帮助用户进行个人财务规划
4. 分析市场趋势，但要明确指出投资有风险

重要原则：
- 始终提醒用户"投资有风险，入市需谨慎"
- 不提供具体的买入卖出建议，只做分析
- 建议用户在重大决策前咨询持牌理财顾问
- 使用专业但易懂的语言

回复风格：
- 专业、严谨、客观
- 适当使用金融术语但要解释清楚
- 提供实用的建议和思路"""

    @property
    def name(self) -> str:
        return self._name
    
    @property
    def description(self) -> str:
        return self._description
    
    def can_handle(self, message: Message, context: ChatContext) -> float:
        """判断是否能处理此消息"""
        if message.has_mention(self.name):
            return 1.0
        
        content = message.content.lower()
        
        # 统计关键词匹配
        keyword_matches = sum(1 for kw in self._finance_keywords if kw in content)
        
        if keyword_matches >= 3:
            return 0.9
        elif keyword_matches == 2:
            return 0.75
        elif keyword_matches == 1:
            return 0.55
        
        return 0.0
    
    async def _generate_llm_response(self, message: str, user_context: Dict) -> str:
        """使用LLM生成专业回复"""
        if not self._llm_provider:
            return None
        
        try:
            context_info = ""
            if user_context.get("risk_preference"):
                context_info += f"\n用户风险偏好: {user_context['risk_preference']}"
            if user_context.get("investment_experience"):
                context_info += f"\n投资经验: {user_context['investment_experience']}"
            
            messages = [
                {"role": "system", "content": self._system_prompt + context_info},
                {"role": "user", "content": message}
            ]
            
            response = await self._llm_provider.generate_response(messages, context=None)
            return response
        except Exception:
            return None
    
    def respond(self, message: Message, context: ChatContext) -> AgentResponse:
        """生成金融咨询回复"""
        user_memory = self.memory_read(message.user_id)
        interaction_count = user_memory.get("interaction_count", 0)
        
        content = message.get_clean_content().lower()
        
        # 识别主题并生成响应
        if any(word in content for word in ["股票", "stock", "证券"]):
            topic = "stock"
            response = self._respond_stock(message, user_memory)
        elif any(word in content for word in ["基金", "fund", "定投"]):
            topic = "fund"
            response = self._respond_fund(message, user_memory)
        elif any(word in content for word in ["理财", "储蓄", "预算", "规划"]):
            topic = "planning"
            response = self._respond_planning(message, user_memory)
        elif any(word in content for word in ["比特币", "加密", "crypto", "bitcoin"]):
            topic = "crypto"
            response = self._respond_crypto(message, user_memory)
        else:
            topic = "general"
            response = self._respond_general(message, interaction_count)
        
        # 更新用户记忆
        user_memory["interaction_count"] = interaction_count + 1
        user_memory["last_topic"] = topic
        user_memory["last_query"] = message.content[:100]
        self.memory_write(message.user_id, user_memory)
        
        return AgentResponse(
            content=response,
            agent_name=self.name,
            confidence=0.85,
            metadata={"topic": topic},
            should_continue=False
        )
    
    def _respond_stock(self, message: Message, user_memory: Dict) -> str:
        """股票相关响应"""
        return (
            "📈 **股票投资咨询**\n\n"
            "关于您的股票问题，我来为您分析：\n\n"
            "**投资股票前需要考虑的因素：**\n"
            "1. 公司基本面分析（财报、行业地位）\n"
            "2. 技术面分析（价格走势、成交量）\n"
            "3. 宏观经济环境\n"
            "4. 个人风险承受能力\n\n"
            "**建议步骤：**\n"
            "• 先了解目标公司的业务模式\n"
            "• 分析近几年的财务数据\n"
            "• 评估当前估值是否合理\n"
            "• 制定投资计划和止损策略\n\n"
            "⚠️ **风险提示**：股市有风险，投资需谨慎。"
            "建议您在做重大投资决策前咨询专业的持牌理财顾问。\n\n"
            "您想了解哪方面的更多细节？"
        )
    
    def _respond_fund(self, message: Message, user_memory: Dict) -> str:
        """基金相关响应"""
        return (
            "💼 **基金投资指南**\n\n"
            "基金是一种适合普通投资者的工具，我来帮您了解：\n\n"
            "**常见基金类型：**\n"
            "• 货币基金：风险低，流动性好\n"
            "• 债券基金：风险适中，收益稳定\n"
            "• 混合基金：股债搭配，平衡风险\n"
            "• 股票基金：收益潜力大，风险较高\n"
            "• 指数基金：跟踪指数，费用低廉\n\n"
            "**定投策略建议：**\n"
            "1. 选择波动较大的基金更适合定投\n"
            "2. 坚持长期投资，避免频繁操作\n"
            "3. 设定目标收益率，达到后适当止盈\n\n"
            "📊 您目前的投资目标和风险偏好是什么？"
            "我可以为您提供更有针对性的建议。"
        )
    
    def _respond_planning(self, message: Message, user_memory: Dict) -> str:
        """理财规划响应"""
        return (
            "📋 **个人财务规划建议**\n\n"
            "科学的理财规划是实现财务自由的基础：\n\n"
            "**理财四步法：**\n\n"
            "**1️⃣ 记账与预算**\n"
            "• 了解收支情况\n"
            "• 制定合理预算\n"
            "• 控制非必要支出\n\n"
            "**2️⃣ 建立应急基金**\n"
            "• 3-6个月生活费\n"
            "• 放在货币基金等流动性好的产品\n\n"
            "**3️⃣ 保险配置**\n"
            "• 医疗险、意外险优先\n"
            "• 根据家庭情况配置寿险\n\n"
            "**4️⃣ 投资增值**\n"
            "• 根据风险偏好选择产品\n"
            "• 分散投资，定期复盘\n\n"
            "💡 您现在处于哪个阶段？我可以给出更具体的建议。"
        )
    
    def _respond_crypto(self, message: Message, user_memory: Dict) -> str:
        """加密货币响应"""
        return (
            "🔗 **加密货币投资须知**\n\n"
            "加密货币是高风险高波动的资产类别：\n\n"
            "**了解风险：**\n"
            "• 价格波动极大（可能一天涨跌20%+）\n"
            "• 监管政策不确定性\n"
            "• 技术风险（私钥丢失、交易所风险）\n"
            "• 市场操纵风险\n\n"
            "**如果要参与，建议：**\n"
            "1. 只投入能承受全部亏损的资金\n"
            "2. 深入了解底层技术和项目\n"
            "3. 使用可靠的交易平台\n"
            "4. 学习安全存储方法\n"
            "5. 不要追涨杀跌\n\n"
            "⚠️ **重要提醒**：加密货币不适合所有人。"
            "请确保您充分了解风险后再做决定。\n\n"
            "您想了解什么具体问题？"
        )
    
    def _respond_general(self, message: Message, interaction_count: int) -> str:
        """通用金融响应"""
        if interaction_count == 0:
            return (
                "💰 **欢迎咨询金融理财服务**\n\n"
                "我是您的金融理财顾问，可以帮助您：\n\n"
                "📈 投资咨询\n"
                "• 股票、基金、债券分析\n"
                "• 投资组合建议\n\n"
                "📋 财务规划\n"
                "• 个人预算管理\n"
                "• 退休规划\n"
                "• 保险配置建议\n\n"
                "🔍 市场分析\n"
                "• 宏观经济解读\n"
                "• 行业趋势分析\n\n"
                "请告诉我您想了解什么？"
            )
        else:
            return (
                "💼 我随时准备为您解答金融理财问题。\n\n"
                "无论是投资策略、理财产品，还是财务规划，"
                "都可以跟我交流。\n\n"
                "请问有什么我可以帮助您的？"
            )
    
    def memory_read(self, user_id: str) -> Dict[str, Any]:
        return self._memory.read(self.name, user_id)
    
    def memory_write(self, user_id: str, data: Dict[str, Any]) -> None:
        self._memory.write(self.name, user_id, data)
