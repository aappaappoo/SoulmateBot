"""
健康顾问Agent

提供健康咨询、运动建议、饮食指导等服务。
这是一个具有高商业价值的Agent，适用于健康管理领域。
"""
from typing import Dict, Any, Optional
from datetime import datetime
from src.agents import BaseAgent, Message, ChatContext, AgentResponse, SQLiteMemoryStore


class HealthAgent(BaseAgent):
    """
    健康顾问Agent - 提供健康生活指导
    
    专长领域:
    - 健康咨询：常见健康问题解答
    - 运动指导：健身计划、运动建议
    - 饮食管理：营养知识、饮食规划
    - 睡眠改善：睡眠质量优化建议
    - 心理健康：压力管理、情绪调节
    
    适用场景:
    - "最近老是失眠怎么办？"
    - "帮我制定一个减肥计划"
    - "如何改善久坐带来的颈椎问题？"
    """
    
    def __init__(self, memory_store=None, llm_provider=None):
        """
        初始化健康顾问Agent
        """
        self._name = "HealthAgent"
        self._description = (
            "提供健康生活指导和建议。"
            "涵盖运动健身、饮食营养、睡眠改善等领域，"
            "帮助用户建立健康的生活方式。"
        )
        self._memory = memory_store or SQLiteMemoryStore()
        self._llm_provider = llm_provider
        
        # 健康相关关键词
        self._health_keywords = [
            # 身体健康
            "健康", "身体", "体检", "症状", "疾病", "预防",
            "health", "body", "symptom", "disease", "prevention",
            
            # 运动健身
            "运动", "健身", "锻炼", "减肥", "减脂", "增肌",
            "exercise", "workout", "fitness", "gym", "weight loss",
            "跑步", "游泳", "瑜伽", "力量训练",
            
            # 饮食营养
            "饮食", "营养", "卡路里", "蛋白质", "维生素",
            "diet", "nutrition", "calories", "protein", "vitamin",
            "吃什么", "食物", "健康饮食",
            
            # 睡眠
            "睡眠", "失眠", "睡不好", "睡不着", "熬夜",
            "sleep", "insomnia", "tired", "fatigue",
            
            # 常见问题
            "头痛", "颈椎", "腰痛", "眼睛", "视力",
            "headache", "neck", "back pain", "eyes",
            
            # 心理健康
            "压力", "焦虑", "放松", "冥想",
            "stress", "anxiety", "relaxation", "meditation",
        ]
        
        # 系统提示词
        self._system_prompt = """你是一位专业的健康生活顾问，拥有丰富的健康管理知识。

你的职责：
1. 提供科学的健康生活建议
2. 解答运动健身、饮食营养相关问题
3. 帮助用户改善睡眠质量
4. 提供压力管理和放松建议

重要原则：
- 始终强调"如有严重症状请及时就医"
- 不提供医疗诊断，只做健康指导
- 建议因人而异，提醒用户根据自身情况调整
- 推荐循序渐进的改变方式

回复风格：
- 温暖、专业、有鼓励性
- 提供具体可行的建议
- 适当使用emoji增加亲和力"""

    @property
    def name(self) -> str:
        return self._name
    
    @property
    def description(self) -> str:
        return self._description
    
    def can_handle(self, message: Message, context: ChatContext) -> float:
        """判断是否能处理此消息"""
        if message.has_mention(self.name):
            return 1.0
        
        content = message.content.lower()
        keyword_matches = sum(1 for kw in self._health_keywords if kw in content)
        
        if keyword_matches >= 3:
            return 0.9
        elif keyword_matches == 2:
            return 0.75
        elif keyword_matches == 1:
            return 0.55
        
        return 0.0
    
    def respond(self, message: Message, context: ChatContext) -> AgentResponse:
        """生成健康咨询回复"""
        user_memory = self.memory_read(message.user_id)
        interaction_count = user_memory.get("interaction_count", 0)
        
        content = message.get_clean_content().lower()
        
        # 识别主题并生成响应
        if any(word in content for word in ["运动", "健身", "锻炼", "减肥", "增肌", "gym"]):
            topic = "fitness"
            response = self._respond_fitness(message, user_memory)
        elif any(word in content for word in ["饮食", "营养", "吃", "食物", "卡路里"]):
            topic = "nutrition"
            response = self._respond_nutrition(message, user_memory)
        elif any(word in content for word in ["睡眠", "失眠", "睡不好", "熬夜", "sleep"]):
            topic = "sleep"
            response = self._respond_sleep(message, user_memory)
        elif any(word in content for word in ["压力", "焦虑", "放松", "stress", "冥想"]):
            topic = "mental"
            response = self._respond_mental(message, user_memory)
        elif any(word in content for word in ["颈椎", "腰", "久坐", "眼睛"]):
            topic = "office"
            response = self._respond_office_health(message, user_memory)
        else:
            topic = "general"
            response = self._respond_general(message, interaction_count)
        
        # 更新用户记忆
        user_memory["interaction_count"] = interaction_count + 1
        user_memory["last_topic"] = topic
        user_memory["health_goals"] = user_memory.get("health_goals", [])
        self.memory_write(message.user_id, user_memory)
        
        return AgentResponse(
            content=response,
            agent_name=self.name,
            confidence=0.85,
            metadata={"topic": topic},
            should_continue=False
        )
    
    def _respond_fitness(self, message: Message, user_memory: Dict) -> str:
        """运动健身响应"""
        return (
            "💪 **运动健身指导**\n\n"
            "我来帮您规划运动方案：\n\n"
            "**新手入门建议：**\n"
            "• 每周运动3-4次，每次30-60分钟\n"
            "• 有氧+力量训练相结合\n"
            "• 循序渐进，不要急于求成\n\n"
            "**运动类型推荐：**\n"
            "🏃 有氧运动：跑步、游泳、骑车（燃脂）\n"
            "🏋️ 力量训练：深蹲、俯卧撑、硬拉（增肌）\n"
            "🧘 柔韧训练：瑜伽、拉伸（恢复）\n\n"
            "**重要提醒：**\n"
            "• 运动前热身5-10分钟\n"
            "• 运动后拉伸放松\n"
            "• 保证充足睡眠和营养\n"
            "• 如有不适立即停止\n\n"
            "📊 您目前的运动基础和目标是什么？"
        )
    
    def _respond_nutrition(self, message: Message, user_memory: Dict) -> str:
        """饮食营养响应"""
        return (
            "🥗 **健康饮食指南**\n\n"
            "科学的饮食是健康的基础：\n\n"
            "**均衡营养原则：**\n"
            "• 碳水化合物：全谷物、蔬菜为主\n"
            "• 蛋白质：鱼肉、禽肉、豆类、蛋奶\n"
            "• 脂肪：坚果、橄榄油等优质脂肪\n"
            "• 膳食纤维：蔬菜、水果、粗粮\n\n"
            "**每日建议：**\n"
            "🥦 蔬菜：400-500g\n"
            "🍎 水果：200-350g\n"
            "🍚 主食：适量全谷物\n"
            "🥛 蛋白质：每餐一掌心\n"
            "💧 饮水：1.5-2升\n\n"
            "**饮食习惯建议：**\n"
            "• 三餐规律，细嚼慢咽\n"
            "• 少油少盐少糖\n"
            "• 避免深加工食品\n\n"
            "🎯 您想改善哪方面的饮食习惯？"
        )
    
    def _respond_sleep(self, message: Message, user_memory: Dict) -> str:
        """睡眠改善响应"""
        return (
            "😴 **改善睡眠质量**\n\n"
            "良好的睡眠对健康至关重要：\n\n"
            "**睡眠环境优化：**\n"
            "• 保持卧室凉爽（18-22℃）\n"
            "• 确保黑暗和安静\n"
            "• 选择舒适的床垫和枕头\n"
            "• 移除电子设备干扰\n\n"
            "**睡前习惯建议：**\n"
            "🌙 睡前1小时避免蓝光\n"
            "☕ 下午4点后避免咖啡因\n"
            "📵 睡前放下手机\n"
            "🛁 温水泡脚或洗澡放松\n"
            "📖 可以阅读或冥想\n\n"
            "**作息建议：**\n"
            "• 固定睡眠和起床时间\n"
            "• 每天保证7-9小时睡眠\n"
            "• 避免周末补觉过多\n\n"
            "⚠️ 如果持续失眠超过2周，建议就医检查。\n\n"
            "您目前的睡眠问题主要是什么？"
        )
    
    def _respond_mental(self, message: Message, user_memory: Dict) -> str:
        """心理健康响应"""
        return (
            "🧘 **压力管理与心理调节**\n\n"
            "关注心理健康同样重要：\n\n"
            "**日常减压方法：**\n"
            "• 🌬️ 深呼吸练习：4-7-8呼吸法\n"
            "• 🧘 冥想：每天10-15分钟\n"
            "• 🚶 散步：户外活动放松身心\n"
            "• ✍️ 写日记：记录感受和想法\n"
            "• 🎵 听音乐：舒缓的音乐疗愈\n\n"
            "**健康心态培养：**\n"
            "• 接受不完美\n"
            "• 专注当下\n"
            "• 与亲友保持联系\n"
            "• 设定合理的期望\n"
            "• 学会说"不"\n\n"
            "**4-7-8呼吸法：**\n"
            "吸气4秒 → 屏息7秒 → 呼气8秒\n"
            "重复3-4次，有助于快速放松\n\n"
            "💙 如果持续感到焦虑或抑郁，建议寻求专业心理帮助。\n\n"
            "您想尝试哪种放松方式？"
        )
    
    def _respond_office_health(self, message: Message, user_memory: Dict) -> str:
        """办公室健康响应"""
        return (
            "💼 **久坐办公健康指南**\n\n"
            "长时间坐着工作需要特别注意：\n\n"
            "**颈椎保护：**\n"
            "• 屏幕与眼睛平齐\n"
            "• 每1小时活动颈部\n"
            "• 做颈部旋转和侧屈\n\n"
            "**腰部保护：**\n"
            "• 坐姿保持腰部支撑\n"
            "• 站起来走动休息\n"
            "• 做腰部拉伸运动\n\n"
            "**眼睛保护（20-20-20法则）：**\n"
            "每20分钟 → 看20英尺外 → 20秒\n\n"
            "**办公室拉伸动作：**\n"
            "1️⃣ 肩部环绕\n"
            "2️⃣ 胸部扩展\n"
            "3️⃣ 坐姿转体\n"
            "4️⃣ 手腕伸展\n"
            "5️⃣ 站起来深蹲\n\n"
            "⏰ 设置提醒每小时起来活动2-3分钟。\n\n"
            "您主要是哪个部位不舒服？"
        )
    
    def _respond_general(self, message: Message, interaction_count: int) -> str:
        """通用健康响应"""
        if interaction_count == 0:
            return (
                "🏥 **欢迎咨询健康生活服务**\n\n"
                "我是您的健康生活顾问，可以帮助您：\n\n"
                "💪 运动健身\n"
                "• 制定运动计划\n"
                "• 健身指导建议\n\n"
                "🥗 饮食营养\n"
                "• 健康饮食指导\n"
                "• 营养知识科普\n\n"
                "😴 睡眠改善\n"
                "• 失眠问题调节\n"
                "• 睡眠习惯优化\n\n"
                "🧘 心理健康\n"
                "• 压力管理\n"
                "• 放松技巧\n\n"
                "请告诉我您想了解什么健康话题？"
            )
        else:
            return (
                "🌿 我随时准备为您解答健康生活问题。\n\n"
                "无论是运动健身、饮食营养、睡眠改善，"
                "还是压力管理，都可以跟我交流。\n\n"
                "⚠️ 温馨提醒：如有严重症状，请及时就医。\n\n"
                "请问有什么我可以帮助您的？"
            )
    
    def memory_read(self, user_id: str) -> Dict[str, Any]:
        return self._memory.read(self.name, user_id)
    
    def memory_write(self, user_id: str, data: Dict[str, Any]) -> None:
        self._memory.write(self.name, user_id, data)
