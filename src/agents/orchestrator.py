...
class IntentType(str, Enum):
    ...
    AGENTS_RESPONSE = "agents_response"  # 需要单个Agent处理
...
- agents_response → agents 填写需要调用的 Agent 名称
...
"""
    ...
    def __init__(
        ...
        Args:
            ...
            enable_unified_mode: 是否启用统一分析模式
        """
...
{unified_task_prompt}"""
            ...
            if not response:
                ...
                logger.debug(
                    f"📝 [Orchestrator] Last message content preview: {messages[-1].get('content', '')[:200]}...")
            ...
            if emotion and emotion in self.SUPPORTED_EMOTIONS:
                ...
                logger.debug(
                    f"🎭 [EMOTION EXTRACT] Extracted emotion from LLM response: emotion={emotion}, emotion_description={emotion_description}")
            ...
            if conversation_summary:
                ...
                if isinstance(conversation_summary, dict):
                    ...
                    if all(field in conversation_summary for field in required_fields):
                        ...
                        logger.debug(
                            f"📝 [SUMMARY] Generated summary: {conversation_summary.get('summary_text', '')[:50]}...")
                    else:
                        logger.warning(
                            f"📝 [SUMMARY] Incomplete summary structure, missing fields: {[f for f in required_fields if f not in conversation_summary]}")
                else:
        ...
        if self.enable_unified_mode and self.llm_provider:
            ...
            if result.intent_type == IntentType.DIRECT_RESPONSE:
                if direct_reply:
                    ...
                    final_response = "DIRECT_RESPONSE 无回复；请联系管理员"
                ...
                if result.selected_agents:
                    ...
                    result.final_response = "IntentType 或者 SelectedAgents出现问题"
                    return result
