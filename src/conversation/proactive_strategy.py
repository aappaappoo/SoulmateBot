"""
Proactive Dialogue Strategy - 主动对话策略系统

让 Bot 在对话中更加主动，主动寻找和用户的共同爱好和话题，而不是被动回应。

核心理念：
- 被动模式（原来）：用户说什么 → 回应什么
- 主动模式（新的）：用户说什么 → 回应 + 主动探索 + 引导话题 + 寻找共鸣

主动模式类型：
- EXPLORE_INTEREST: 开场/探索阶段，主动问用户兴趣爱好
- DEEPEN_TOPIC: 发现用户感兴趣的话题，追问细节
- SHARE_AND_ASK: 关系已建立，分享自己的想法再提问
- FIND_COMMON: 发现共同点，积极表达共鸣
- SHOW_CURIOSITY: 用户很积极，表达好奇
- RECALL_MEMORY: 有待追问的线索，回忆之前的事
- SUPPORTIVE: 用户情绪低落，倾听支持
- GENTLE_GUIDE: 用户正在脱离，简短回应
"""
...
class ProactiveMode(str, Enum):
    """主动对话模式"""
    EXPLORE_INTEREST = "explore_interest"  # 探索兴趣
    DEEPEN_TOPIC = "deepen_topic"          # 深入话题
    SHARE_AND_ASK = "share_and_ask"        # 分享并提问
    FIND_COMMON = "find_common"            # 寻找共同点
    SHOW_CURIOSITY = "show_curiosity"      # 表达好奇
    RECALL_MEMORY = "recall_memory"        # 回忆并追问
    SUPPORTIVE = "supportive"              # 支持倾听
    GENTLE_GUIDE = "gentle_guide"          # 温和引导
...
# 兴趣关键词库
INTEREST_CATEGORIES = {
    "游戏": ["游戏", "打游戏", "玩游戏", "王者", "吃鸡", "LOL", "原神", "switch"],
    "影视": ["电影", "电视剧", "动漫", "看片", "追剧", "综艺"],
    "音乐": ["音乐", "听歌", "唱歌", "KTV", "歌手", "演唱会"],
    "运动": ["运动", "健身", "跑步", "游泳", "瑜伽", "篮球", "足球"],
    "阅读": ["看书", "读书", "小说", "漫画", "阅读"],
    "旅游": ["旅游", "旅行", "出去玩", "度假", "景点"],
    "美食": ["吃", "美食", "好吃", "餐厅", "零食", "做饭"],
    "手工": ["手工", "DIY", "画画", "手账", "乐高"],
...
# 主动提问模板
PROACTIVE_QUESTIONS = {
    ProactiveMode.EXPLORE_INTEREST: [
        "对了，你平时喜欢做什么呀？有什么爱好吗？",
        "说起来，你是什么星座的呀？我挺好奇的~",
        "你平时喜欢什么类型的{category}呀？",
        "最近心情怎么样呀？有什么开心或者烦心的事吗？"
    ],
    ProactiveMode.DEEPEN_TOPIC: [
        "听起来很有意思！能跟我详细说说{topic}吗？",
        "哇，{topic}！你是什么时候开始喜欢的呀？",
        "我也对{topic}很感兴趣！你最喜欢的是什么？",
        "那你觉得{topic}最吸引你的地方是什么呢？"
    ],
    ProactiveMode.SHARE_AND_ASK: [
        "我也{interest}！不过我更喜欢{detail}，你呢？",
        "说起{topic}，我之前也有类似的经历，你是怎么处理的？",
        "对于{topic}，我的想法是{opinion}，你怎么看？"
    ],
    ProactiveMode.FIND_COMMON: [
        "我也{interest}！咱们可以一起{activity}呀~",
        "原来你也喜欢{interest}！我也是！",
        "哇，{interest}同好！你平时{specific_activity}吗？"
    ],
    ...
    ProactiveMode.RECALL_MEMORY: [
        "对了，你之前提到过{memory_topic}，后来怎么样了？",
        "记得你说过{memory_content}，最近还有在{activity}吗？",
        "你上次说的{memory_topic}，我一直记得呢，现在情况怎么样？"
    ]
}
...
class ProactiveDialogueStrategyAnalyzer:
    ...
    def analyze_topic(
        ...
        # 识别共同兴趣
        analysis.common_interests = user_profile.interests[:3]  # 简化：前3个兴趣
    ...
    def generate_proactive_strategy(
        ...
        # 根据情绪和阶段选择模式
        mode = self._select_proactive_mode(
            stage, user_profile, topic_analysis, user_memories
        )
        
        # 生成具体建议
        action = self._build_proactive_action(
            mode, user_profile, topic_analysis, user_memories
        )
    ...
    def _analyze_emotional_state(self, conversation_history: List[Dict[str, str]]) -> str:
        ...
        # 情绪关键词
        positive_keywords = ["开心", "高兴", "喜欢", "爱", "棒", "好", "不错"]
        negative_keywords = ["难过", "伤心", "焦虑", "累", "烦", "失落", "孤独"]
    ...
    def _extract_recent_topics(self, conversation_history: List[Dict[str, str]]) -> List[str]:
        ...
        topic_keywords = {
            ...
            "学习": ["学习", "考试", "作业"],
            "情感": ["喜欢", "爱", "恋爱"],
            "兴趣": list(self.interest_categories.keys())
        }
