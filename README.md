...
| ⏰**定时提醒**     | 支持用户设置提醒，Bot主动推送提醒消息   |
| 📄**配置文件优先** | YAML配置文件优先，支持灵活的Bot个性配置 |
...
每次调用 LLM API 时，系统会构建一个完整的消息列表发送给 LLM。消息结构如下：

#### 消息组成

1. **System Message（系统消息）**：包含以下内容
   - 🤖 **Bot 人设定义**：来自配置文件的角色设定
   - 💾 **用户记忆**：检索到的用户长期记忆（偏好、重要事件等）
   - 💬 **对话摘要**：中期对话的摘要信息
   - 🎯 **对话策略**：根据对话阶段和用户情绪生成的策略指导
   - 📋 **任务指令（UNIFIED_PROMPT_TEMPLATE）**：
     - 任务1：意图识别（判断消息类型）
     - 任务2：生成回复（直接回复用户）
     - 任务3：对话摘要生成（提取关键要素）
     - 任务4：记忆分析（判断是否值得记住）
   - 📊 **返回格式要求**：JSON 格式的结构化输出要求

2. **Conversation History（对话历史）**：最近 5 轮对话（约 10 条消息）
   - 交替的 user/assistant 消息
   - 用于保持对话上下文连贯性

3. **Current User Message（当前用户消息）**：用户的当前输入
...
```
🚀 [AI-REQ][{request_id}] provider=... | model=... | messages=6 | chars=...
📤 [AI-RES][{request_id}] full_response: ...
```

使用 DEBUG 日志级别可以看到完整的请求和响应内容。
...
SoulmateBot 支持用户设置定时提醒。用户可以通过自然语言告诉 Bot 在指定时间后提醒自己做某事，Bot 会在到期时主动发送提醒消息。
...
- "in 1 hour remind me to call John"

### 支持的时间单位

| 单位 | 中文 | 英文 |
|------|------|------|
| 分钟 | 分钟、分 | minute(s), min |
| 小时 | 小时、个小时 | hour(s), hr, h |
| 天 | 天 | day(s) |
...
SoulmateBot 支持使用 YAML 配置文件来定义 Bot 的个性。当 `bots/` 目录中存在对应的配置文件时，系统会优先使用文件配置，否则使用数据库中的配置。

### 配置文件位置

每个 Bot 的配置文件放置在 `bots/<bot_name>/config.yaml`：
...
当配置文件中包含 `personality` 配置时，用户首次使用 `/start` 命令时，Bot 会展示个性化的欢迎消息，包括：
...
- ✅ YAML 配置文件优先：当 `bots/` 目录中存在 Bot 配置文件时，优先使用文件配置加载
- ✅ 个性化欢迎消息：用户首次使用 `/start` 时，Bot 会主动介绍自己的性格、爱好、理想等信息
- ✅ 定时提醒功能：用户可以说"1小时后提醒我做某事"，Bot 会在指定时间主动发送提醒
- ✅ 智能提醒解析：支持中文和英文的多种提醒格式，支持分钟/小时/天等时间单位
